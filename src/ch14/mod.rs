...
fn process_args(args: &Ch14Args) -> (Box<dyn Optimizer>, NetworkConfig) {
    ...
    let (name, optimizer): (String, Box<dyn Optimizer>) = match args.command {
        cli::OptimizerCommand::SDG(SDGCommand {
            ...
            format!("ch14-2x{}-sdg-l{}-d{}-m{}", args.layer_neurons, learning_rate, decay_rate, momentum),
...
pub fn run(args: Ch14Args) {
    ...
    let mut network = Network::new(num_labels, args.layer_neurons, args.l2reg);
    ...
    for epoch in 0..config.num_epochs {
        ...
        let NetworkOutput(data_loss, regularization_loss, prediction) = network.forward(&data, &labels);
...
Test Data Loss: {:.3}, Test Regularization Loss: {:.3}, Test Accuracy: {:.3}",
                epoch,
                data_loss,
                regularization_loss,
                accuracy,
