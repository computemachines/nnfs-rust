...
pub struct Network {
    ...
    pub loss: BinaryCrossentropy,
}
pub struct NetworkOutput(pub f64, pub f64, pub Array2<f64>);

impl Network {
    pub fn new(num_neurons_layer: usize, l2reg: f64, dropout: f64) -> Self {
        ...
        // layer) and 1 output value (output tag)
        let mut dense2 = LayerDense::new(num_neurons_layer, 1);
    ...
    pub fn forward(&mut self, input: &Array2<f64>, y_true: &Array2<f64>) -> NetworkOutput {
        ...
        self.activation2.forward(self.dense2.output.as_ref().unwrap());
        let data_loss = self.loss.calculate(self.activation2.output.as_ref().unwrap(), y_true);
        ...
        let mut prediction = self.activation2.output.as_ref().unwrap().mapv(|x| if x > 0.5 {1.0} else {0.0});

        NetworkOutput(data_loss, regularization_loss, prediction)
    }
    pub fn validate(&self, input: &Array2<f64>, y_true: &Array2<f64>) -> NetworkOutput {
        ...
        this.activation2.forward(this.dense2.output.as_ref().unwrap());

        let data_loss = this.loss.calculate(this.activation2.output.as_ref().unwrap(), y_true);
        ...
        let mut prediction = this.activation2.output.as_ref().unwrap().mapv(|x| if x > 0.5 {1.0} else {0.0});

        NetworkOutput(data_loss, regularization_loss, prediction)
    }
    pub fn backward(&mut self, prediction: &Array2<f64>, y_true: &Array2<f64>) {
        self.loss.backward(self.activation2.output.as_ref().unwrap(), y_true);
        self.activation2.backward(self.loss.dinputs.as_ref().unwrap());
        self.dense2
        ...
        self.dense1.backward(self.activation1.dinputs.as_ref().unwrap());
    }
}
